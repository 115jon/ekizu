...
detail::unencrypted_frame_header_size bytes_covering_h264_pps(
	const uint8_t *payload, const uint64_t size_remaining) {
	...
	while (payload_bit_index < size_remaining * 8 &&
		   parsed_exp_golomb_values < 3) {
		...
		if ((payload_byte & (1 << (7 - bit_index))) == 0) {
			...
			if (zero_bit_count >= 32) { return 0; }
		} else {
			// we hit a one
			// skip forward the number of bits dictated by the leading number of
			// zeroes
	...
	if (result >
		std::numeric_limits<detail::unencrypted_frame_header_size>::max()) {
		// bytes covering H264 PPS result cannot fit into unencrypted frame
		// header size
		return 0;
	}
	return static_cast<detail::unencrypted_frame_header_size>(result);
}
...
std::optional<index_start_code_size_pair> next_h26x_nalu_index(
	const uint8_t *buffer, const size_t buffer_size,
	const size_t search_start_index = 0) {
	...
	if (buffer_size < NALU_SHORT_START_SEQUENCE_SIZE) { return std::nullopt; }

	// look for NAL unit 3 or 4 byte start code
	for (size_t i = search_start_index;
		...
		if (buffer[i + 2] > start_code_highest_possible_value) {
			...
			if (buffer[i + 1] == start_code_leading_bytes_value &&
				...
				if (i >= 1 && buffer[i - 1] == start_code_leading_bytes_value) {
					// 4 byte start code
					return std::make_optional<index_start_code_size_pair>(
						{nal_unit_start_index, 4});
				}
				// 3 byte start code
				return std::make_optional<index_start_code_size_pair>(
					{nal_unit_start_index, 3});
			}
...
bool process_frame_vp8(OutboundFrameProcessor &processor,
	...
	// if this is a key frame the depacketizer will read 10 bytes into the
	// payload header if this is a delta frame the depacketizer only needs the
	// first byte of the payload header (since that's where the key frame flag
	// is)
	...
	processor.add_unencrypted_bytes(frame.first(unencrypted_header_bytes));
	processor.add_encrypted_bytes(frame.subspan(unencrypted_header_bytes));
	return true;
}
...
bool process_frame_h264(OutboundFrameProcessor &processor,
						boost::span<const uint8_t> frame) {
	// minimize the amount of unencrypted header data for H264 depending on the
	// NAL unit type from WebRTC, see:
	...
	while (nalu_index_pair && nalu_index_pair->first < frame.size() - 1) {
		...
		auto nal_type =
			frame.data()[nal_unit_start_index] & nal_header_type_mask;

		// copy the start code and then the NAL unit

		// Because WebRTC will convert them all start codes to 4-byte on the
		// receiver side always write a long start code and then the NAL unit
		processor.add_unencrypted_bytes(
			boost::span<const uint8_t>(NALU_LONG_START_CODE));

		auto next_nalu_index_pair = next_h26x_nalu_index(
			frame.data(), frame.size(), nal_unit_start_index);
		auto next_nalu_start =
			next_nalu_index_pair.has_value()
				? next_nalu_index_pair->first - next_nalu_index_pair->second
				: frame.size();
		...
		if (nal_type == nal_type_slice || nal_type == nal_type_idr) {
			...
			auto nal_unit_payload_start =
				nal_unit_start_index + nal_unit_header_size;
			auto nal_unit_pps_bytes = bytes_covering_h264_pps(
				frame.data() + nal_unit_payload_start,
				frame.size() - nal_unit_payload_start);

			processor.add_unencrypted_bytes(frame.subspan(
				nal_unit_start_index,
				nal_unit_header_size + nal_unit_pps_bytes));
			processor.add_encrypted_bytes(frame.subspan(
				nal_unit_start_index + nal_unit_header_size +
					nal_unit_pps_bytes,
			...
			processor.add_unencrypted_bytes(frame.subspan(
				nal_unit_start_index, next_nalu_start - nal_unit_start_index));
		}
...
bool process_frame_h265(OutboundFrameProcessor &processor,
						boost::span<const uint8_t> frame) {
	// minimize the amount of unencrypted header data for H265 depending on the
	// NAL unit type from WebRTC, see:
	// src/modules/rtp_rtcp/source/rtp_format_h265.cc
	...
	while (nalu_index && nalu_index->first < frame.size() - 1) {
		...
		uint8_t nal_type =
			(frame.data()[nal_unit_start_index] & nal_header_type_mask) >> 1;
		...
		if (nal_type < nal_type_vcl_cutoff) {
			...
			processor.add_unencrypted_bytes(
				frame.subspan(nal_unit_start_index, nal_unit_header_size));
			processor.add_encrypted_bytes(frame.subspan(
				nal_unit_start_index + nal_unit_header_size,
				next_nalu_start - nal_unit_start_index - nal_unit_header_size));
		} else {
...
bool process_frame_av1(OutboundFrameProcessor &processor,
	...
	while (i < frame.size()) {
		...
		if (obu_has_size) {
			...
			obu_payload_size = read_leb128(ptr, end);
			if (ptr == nullptr) { return false; }
			i += ptr - start;
		} else {
			// If the size is not present, the OBU extends to the end of the
			// frame.
		...
		if (obu_type != obu_type_temporal_delimiter &&
			...
			processor.add_unencrypted_bytes(
				boost::span<const uint8_t>(&obu_header, 1));
			if (obu_has_extension) {
				// write the extension byte unencrypted
				processor.add_unencrypted_bytes(frame.subspan(
					obu_header_index + sizeof(obu_header),
					obu_extension_size_bytes));
			}
			...
			if (obu_has_size && !rewritten_without_size) {
				// The AMD AV1 encoder may pad LEB128 encoded sizes with a zero
				// byte which the webrtc packetizer removes. To prevent the
				// packetizer from changing the frame, we sanitize the size by
				// re-writing it ourselves
				uint8_t leb128Buffer[LEB128_MAX_SIZE];
				size_t additional_bytes_to_write =
					write_leb128(obu_payload_size, leb128Buffer);
				processor.add_unencrypted_bytes(boost::span<const uint8_t>(
					leb128Buffer, additional_bytes_to_write));
			}

			// add the OBU payload, encrypted
			processor.add_encrypted_bytes(
				frame.subspan(obu_payload_index, obu_payload_size));
		}
	}
...
bool validate_encrypted_frame(OutboundFrameProcessor &processor,
	...
	if (codec != Codec::H264 && codec != Codec::H265) { return true; }

	constexpr size_t padding = NALU_SHORT_START_SEQUENCE_SIZE - 1;

	const auto &unencrypted_ranges = processor.get_unencrypted_ranges();

	// H264 and H265 ciphertexts cannot contain a 3 or 4 byte start code {0, 0,
	// 1} otherwise the packetizer gets confused and the frame we get on the
	// decryption side will be shifted and fail to decrypt
	size_t encrypted_section_start{};
	for (const auto &range : unencrypted_ranges) {
		...
		auto start = encrypted_section_start -
					 std::min(encrypted_section_start, size_t{padding});
	...
	if (encrypted_section_start == frame.size()) { return true; }

	auto start = encrypted_section_start -
				 std::min(encrypted_section_start, size_t{padding});
	auto end = frame.size();
	return !static_cast<bool>(
		next_h26x_nalu_index(frame.data() + start, end - start));
}
